...
async function boot(){ME=await api('/api/auth/me');$('#userChip').textContent=`${ME.displayName||ME.username} • ${ME.role.toUpperCase()}`;if(ME.role==='admin')$('#adminLink').classList.remove('hidden');META=await api('/api/meta');const rawDate=META.maxDate?parseISO(META.maxDate):null;$('#updatedAt').textContent=rawDate?(()=>{
  ...
  const updateTime=updateAt&&!Number.isNaN(updateAt.getTime())
    ?updateAt.toLocaleTimeString('id-ID',{timeZone:'Asia/Jakarta',hour:'2-digit',minute:'2-digit',hour12:false}).replace('.',':')
    :'--:--';
  const mode=META.runtime?.lastUpload?.mode;
  const source=mode==='metabase_auto_sync'?'auto update':'manual update';
  return `Update Sales Data ${rawDate.toLocaleDateString('id-ID',{day:'numeric',month:'short',year:'numeric'})} pk ${updateTime} (by ${source})`;
})():'NO RAW DATA';$('#noData').classList.toggle('hidden',!!META.runtime?.rowCount);initState();renderFilters();for(const sec of ['channel','target','categoryTarget','store','brand','item'])await loadSection(sec)}
