...
S.store={periods:clone(p),prevCount:2,product:'',productLabel:'',filters:{pts:['EFM','EFIT','ESB'],stores:clone(common.stores),channels:clone(common.channels),categories:clone(common.categories),brands:clone(common.brands),salesTypes:clone(common.salesTypes),customerTypes:clone(common.customerTypes),storeStats:clone(common.storeStats)}};
S.brand={periods:clone(p),prevCount:2,topN:Math.min(10,META.rankingDefault||10),filters:{channels:clone(common.channels),stores:clone(common.stores),categories:clone(common.categories),salesTypes:clone(common.salesTypes),customerTypes:clone(common.customerTypes),storeStats:clone(common.storeStats)}};
S.item={periods:clone(p),prevCount:2,topN:Math.min(10,META.rankingDefault||10),product:'',productLabel:'',filters:{channels:clone(common.channels),stores:clone(common.stores),categories:clone(common.categories),brands:clone(common.brands),salesTypes:clone(common.salesTypes),customerTypes:clone(common.customerTypes),storeStats:clone(common.storeStats)}};
}
...
function renderStore(d){const ps=periodsFor('store'),n=ps.length;const order=n===3?[0,1,2]:[0,1];let h=`<table class="sortable-table chart-table" data-chart-label="2"><thead><tr><th rowspan="2">STORE STAT</th><th rowspan="2">PT</th><th rowspan="2">STORE</th><th colspan="${n}">SALES</th><th colspan="${n}">TRX</th><th colspan="${n}">BASKET SIZE</th></tr><tr>${['Sales','Trx','Basket'].map(()=>order.map(i=>`<th>${i===0?'Current':'Previous '+i}<span class="period-sub">${rangeLabel(ps[i])}</span></th>`).join('')).join('')}</tr></thead><tbody>`;for(const r of d.rows){h+=`<tr><td>${r.storeStat==='New Store'?'New':'Exist'}</td><td>${esc(r.pt)}</td><td>${esc(r.store)}</td>${[0,1,2].map(metric=>order.map(i=>`<td class="num">${fmt([r.periods[i].sales,r.periods[i].trx,r.periods[i].basket][metric])}</td>`).join('')).join('')}</tr>`}h+=`<tr class="total no-sort-row"><td colspan="3">ALL STORE</td>${[0,1,2].map(metric=>order.map(i=>`<td class="num">${fmt([d.total[i].sales,d.total[i].trx,d.total[i].basket][metric])}</td>`).join('')).join('')}</tr>`;
for(const [label,key,cls] of [['Variance Value','value','summary-blue-label'],['Variance %','pct','summary-blue'],['Avg Per Day','avg','']]){h+=`<tr class="${label==='Avg Per Day'?'avg-row ':''}no-sort-row"><td colspan="3" class="${cls}">${label}</td>`;for(let metric=0;metric<3;metric++){for(const i of order){let v='-';if(label==='Avg Per Day')v=fmt([d.avgPerDay[i].sales,d.avgPerDay[i].trx,d.avgPerDay[i].basket][metric]);else if(i<d.variance.length&&d.variance[i])v=label==='Variance Value'?fmt([d.variance[i].sales,d.variance[i].trx,d.variance[i].basket][metric]):fp([d.variance[i].salesPct,d.variance[i].trxPct,d.variance[i].basketPct][metric]);h+=`<td class="num ${label!=='Avg Per Day'?'summary-blue':''}">${v}</td>`}}h+='</tr>'}
for(const pt of d.ptTotals){h+=`<tr class="no-sort-row"><td colspan="2">${pt.pt}</td><td>PT TOTAL</td>${[0,1,2].map(metric=>order.map(i=>`<td class="num">${fmt([pt.periods[i].sales,pt.periods[i].trx,pt.periods[i].basket][metric])}</td>`).join('')).join('')}</tr>`}h+=`<tr class="pt-total no-sort-row"><td colspan="3">TOTAL</td>${[0,1,2].map(metric=>order.map(i=>`<td class="num">${fmt([d.total[i].sales,d.total[i].trx,d.total[i].basket][metric])}</td>`).join('')).join('')}</tr></tbody></table>`;$('#storeTable').innerHTML=h;const t=$('#storeTable table');t.dataset.chartCols=n===3?'3,4,5':'3,4';t.dataset.chartSeries=n===3?'Current,Previous 1,Previous 2':'Current,Previous 1';enhanceTable(t)}
